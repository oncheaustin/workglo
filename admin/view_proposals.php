...
$count_all_proposals = $db->count("proposals");
...
$get_proposals = $db->query("select * from proposals where not proposal_status='modification' AND not proposal_status='draft' order by 1 DESC LIMIT :limit OFFSET :offset","",array("limit"=>$per_page,"offset"=>$start_from));
...
$query = $db->query("select * from proposals where not proposal_status='modification'");
