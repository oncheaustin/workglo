...
<div class="form-group mt-3 mb-3"><!--- form-group mt-2 Starts --->
	<ul class="buyables m-b-25">
		...
		$i = 0;
		$total = 0;
		while($row_extras = mysqli_fetch_array($run_extras)){
		$id = $row_extras['id'];
...
<form method="post" action="../../checkout" id="checkoutForm"><!--- form Starts --->
  ...
  <?php } ?>
	...
	<strong>
	Order Now (<?php echo $s_currency; ?><span class="total-price"><?php echo $price; ?></span>)
	</strong>
	</button>
	<?php if($count_extras > 0){ ?>
	<div class="form-group mt-3 mb-3"><!--- form-group mt-2 Starts --->
  ...
  while($row_extras = $get_extras->fetch()){
	...
	<li>
	  <label class="fake-check-black check-text">
		<input type="checkbox" name="proposal_extras[<?php echo $i; ?>]" value="<?php echo $id; ?>">
		<span class="chk-img"></span>
		<span class="js-express-delivery-text"><?php echo $name; ?> (+<span class="price"><?php echo $s_currency.$price; ?></span>)</span>
		</label>
	</li>
  <?php } ?>
	</ul>	
